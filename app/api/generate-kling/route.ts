...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await fetch('https://api.kuaishou.com/v1/videos/text2video', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${process.env.KLING_API_KEY}` // Add your Kling API key to .env.local
            },
        ...
        if (!response.ok) {
            console.error('❌ Kling API Error:', responseData);
            return NextResponse.json({
                success: false,
        ...
        if (responseData.code !== 0) {
            console.error('❌ Kling API Response Error:', responseData);
            return NextResponse.json({
                success: false,
