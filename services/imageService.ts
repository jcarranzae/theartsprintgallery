...
import { GenerateImageResponse, ImageResult, FluxProRequest, FluxProUltraRequest, ModelType } from '@/types/api';
import { MODELS } from '@/types/api';

const api = axios.create({
  baseURL: process.env.NEXT_PUBLIC_BFL_BASE_URL,
  headers: {
...
export const imageService = {
  async generateImage(request: FluxProRequest | FluxProUltraRequest, model: ModelType): Promise<GenerateImageResponse> {
   //const zbaseURL= process.env.NEXT_PUBLIC_BFL_BASE_URL
    const endpoint = model === MODELS.FLUX_PRO 
      ? API_CONFIG.ENDPOINTS.FLUX_PRO 
  ...
  async getImageResult(id: string): Promise<ImageResult> {
